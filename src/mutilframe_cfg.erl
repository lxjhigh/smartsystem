%% @author liuheliang
%% @doc @todo Add description to mutilframe_cfg.
%%配置文件下载


-module(mutilframe_cfg).
 
-behaviour(gen_fsm).

%%配置文件类型,不同的配置文件，就是type不一样
-define(TYPE,2).
...
%%===========================================================================
handle_info({reply,16#7F,?TYPE,<<_Frameindex:16,Framecount:16,_Compress:8,_Version:32,Totalength:32,_Crc:32,_Other/binary>>},info,{#static_data{token=Token,client=Client,mac=Mac,ip=Desthost,port=Destport}=Static,Dynamic}) ->
	...
	%%【状态跳跃】
    {next_state,data,{Static,Dynamic#dynamic_data{bindex=1,eindex=Framecount,recdata=[],sendata=Dataframe,time=NewTime,num=1,totalength=Totalength}}};
                          ...
                          true->
                                   ...
                                   case Frameindex >= Eindex of
                                                         true->
                                                               ...
                                                               Content = list_to_binary(Newrecdata),
                                                               %%验证收到的收据总长度是否一致,测试使用
                                                               ...
                                                               Path = Rootpath ++ "/down/cfg_" ++ integer_to_list(Mac) ++ ".bin",
